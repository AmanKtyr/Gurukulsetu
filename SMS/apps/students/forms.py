...
from apps.corecode.models import Section, StudentClass, AcademicSession, AcademicTerm
...
class StudentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        # Extract college from kwargs if provided
        college = kwargs.pop('college', None)
        super().__init__(*args, **kwargs)

        # Filter choices by college if provided
        if college:
            self.fields['current_class'].queryset = StudentClass.objects.filter(college=college)
            self.fields['session'].queryset = AcademicSession.objects.filter(college=college)
            self.fields['term'].queryset = AcademicTerm.objects.filter(college=college)
